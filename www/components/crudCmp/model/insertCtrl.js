...
app.controller('insertCtrl', ['$scope', '$mdDialog', '$http', '$mdToast', function($scope, $mdDialog, $http, $mdToast) {
	...
	$scope.insert = function() {
		for (var key in $scope.dialogOpt.models) {
			if (!$scope.dialogOpt.models[key]) return;
		}
		$http.post(basepath + '/crud/' + $scope.selectedDbs + '/' + $scope.selectedTbl, $scope.dialogOpt.models).then(function(iRes) {
			$mdDialog.hide();
			$mdToast.show(
				$mdToast.simple()
				.content('Succesfully inserted one row')
				.hideDelay(3000)
			);
		}, function(err) {
			showError(err);
